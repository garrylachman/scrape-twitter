...
var toTwitterProfile = function toTwitterProfile(_ref) {
  ...
  var tweetCount = toNumber($nav.find('.ProfileNav-item--tweets .ProfileNav-value').first().text());
  var followingCount = toNumber($nav.find('.ProfileNav-item--following .ProfileNav-value').first().text());
  var followerCount = toNumber($nav.find('.ProfileNav-item--followers .ProfileNav-value').first().text());
  var likeCount = toNumber($nav.find('.ProfileNav-item--favorites .ProfileNav-value').first().text());
...
var toThreadedTweets = function toThreadedTweets(id) {
  return function (_ref4) {
    ...
    ancestorTweetElements.forEach(function (tweetElement, index) {
      var tweet = _extends({}, parseTweet($, tweetElement), { isReplyToId: lastAncestorTweetId });
    ...
    var threadedConversations = threadElements.map(function (threadedConversationElement) {
      ...
      tweetElements.forEach(function (tweetElement, index) {
        ...
        var tweet = _extends({}, parseTweet($, tweetElement), { isReplyToId: lastTweetId, _showMoreTweetsFromConversation: _showMoreTweetsFromConversation });
        tweets.push(tweet);
